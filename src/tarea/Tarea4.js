...
//Solo ejecuta el onchange, sin parámetros, para que sea más general
...
export function CheckboxListWithState(props) {
  const { items } = props;
  const [checkboxes, setCheckboxes] = React.useState(items);
  return (
    <div>
      {Object.entries(checkboxes).map(([name, value], index) => (
        <ControlledCheckbox
          key={index}
          name={name}
          value={value}
